...
namespace WebZSPrev.ObjetoTransferencia
{
    public class StatusCompromisso
    {
        ...
        public List<StatusCompromisso> consulta(string procedure, string metodo)
        {
            try
            {
                acesso.LimpaParametros();
                ...
                List<StatusCompromisso> listaStatusCompromisso = new List<StatusCompromisso>();
                using (dt = acesso.ExecutaConsulta(CommandType.StoredProcedure, procedure))
                    foreach (DataRow dr in dt.Rows)
                    {

                        StatusCompromisso novoStatusCompromisso = new StatusCompromisso();


                        novoStatusCompromisso.id = Convert.ToInt32("id");
                        novoStatusCompromisso.nome = Convert.ToString("nome");
                        novoStatusCompromisso.apagado = Convert.ToInt32("apagado");

                        listaStatusCompromisso.Add(novoStatusCompromisso);
                    }

                return listaStatusCompromisso;
